...
export default function ProductDetail({ product, selectedPicture, reviews, handleAddToCart, setSelectedDetail }) {
    ...
    <div className="product-detail-container">
      ...
      <div className="detail-shower">
        ...
        <div className="star-rating">
          <img src={`main-images/stars/${product?.rate * 10}.svg`} alt="star-rating" />
          <p>{product.rate} {`(${reviews?.length}) Reviews`}</p>
        </div>
        ...
        <div className="color-section">
          ...
          <div className="color-container">
            {product?.colors?.map((c, i) => (
              <span onClick={() => setSelectedDetail(prev => ({...prev, selected_color : c}))} key={i} className={`${c.toLowerCase()}-color`}></span>
            ))}
          </div>
        </div>
        <div className="size-section">
          ...
          <div className="size-container">
            {product?.size?.map((s, i) => (
              <span onClick={() => setSelectedDetail(prev => ({...prev, selected_size : s}))} key={i} className={`${s.toLowerCase()}-size`}>
                {s}
              </span>
            ))}
          </div>
        ...
        <div className="operation-container">
          <div className="count-controller">
            ...
            <p style={{ pointerEvents: "none" }}>{count}</p>
            <div
              style={{ cursor: "pointer" }}
              onClick={() => setCount((prev) => prev + 1)}
            >
